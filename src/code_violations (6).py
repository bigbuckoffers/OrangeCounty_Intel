...
def normalize_oc_parcel(raw_pid):
    """
    Orange County parcel format: SS-TT-RR-SSSS-BB-LLL (Section-Township-Range-Sub-Block-Lot)
    Some sources return parcels with Section/Range swapped: RR-TT-SS-SSSS-BB-LLL.
    This function attempts to detect and correct the swap.

    Detection: OC sections are 01-36, townships 19-23, ranges 27-32.
    If first 2 digits look like a Range (27-32) and last group looks like Section (01-36),
    swap them.
    """
    c = clean_parcel(raw_pid)
    if len(c) != 15:
        return c  # can't normalize non-standard length

    # OC parcel format breakdown (15 digits):
    # Positions 0-1:  Section (01-36)
    # Positions 2-3:  Township (19-23)
    # Positions 4-5:  Range (27-32)
    # Positions 6-9:  Subdivision code
    # Positions 10-11: Block
    # Positions 12-14: Lot suffix
    try:
        sec = int(c[0:2])
        twp = int(c[2:4])
        rng = int(c[4:6])

        # Valid OC ranges: Section 01-36, Township 19-24, Range 27-33
        sec_valid = 1 <= sec <= 36
        twp_valid = 19 <= twp <= 24
        rng_valid = 27 <= rng <= 33

        if sec_valid and twp_valid and rng_valid:
            return c  # already correct order

        # Try swapping Section and Range (positions 0-1 and 4-5)
        swapped = c[4:6] + c[2:4] + c[0:2] + c[6:]
        s2 = int(swapped[0:2])
        r2 = int(swapped[4:6])
        if 1 <= s2 <= 36 and 27 <= r2 <= 33:
            log.info("CV parcel order corrected: %s -> %s", c, swapped)
            return swapped
...
def group_by_property(violations):
    ...
    for v in violations:
        parcel = normalize_oc_parcel(clean_parcel(v.get("Parcel ID", "")))
...
def group_to_lead(group_key, viol_list, enriched):
    first  = viol_list[0]
    count  = len(viol_list)
    parcel = normalize_oc_parcel(first.get("Parcel ID", ""))
...
def main():
    ...
    del data  # free original loaded JSON before writing

    log.info("Writing output.json (%d leads)...", len(leads))
