...
def main():
    ...
    for i, lead in enumerate(leads):
        ...
        needs_prop_street= not lead.get("prop_street") or bool(re.match(r'^\d+$',(lead.get("prop_street","") or "").strip()))
        # Only call OCPA when property address data is actually incomplete
        # Don't waste API calls on leads that already have full address
        needs_ocpa = needs_prop_addr or needs_prop_zip or needs_prop_city or needs_prop_street

        # ── OCPA enrichment ───────────────────────────────────────────────
        if pid and pid.isdigit() and needs_ocpa and ocpa_enriched < MAX_OCPA_PER_RUN:
    ...
    # ── Property-level stacking — runs after ALL sources are loaded ───────
    # This is the final dedup pass. By this point scraper, foreclosure,
    # tax_delinquent, and code_violations have all run, so every signal
    # for every property is in the leads list. We now merge duplicates.
