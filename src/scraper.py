...
def _primary_party(party_str):
    ...
    The actual property owner is always the FIRST name listed.
    Association/HOA/trust names are skipped if they appear first.
    """
    if not party_str:
        return ""
...
def parse_csv_text(csv_text, doc_type, base_score):
    ...
    for row in reader:
        ...
        raw_grantee = row.get("Grantee", "") or ""
        raw_grantor = row.get("Grantor", "") or ""
        # Grantee is always the property owner — they receive the filing against their property
        # Grantor is the filer (HOA, bank, lender, contractor) — never the owner
        owner = _primary_party(raw_grantee) or _primary_party(raw_grantor)
        leads.append(Lead(
