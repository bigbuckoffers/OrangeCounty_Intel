...
def label_match(score, parsed, notes):
    # Real lot/unit anchor — must be actual lot number match, not false unit token
    has_lot_anchor    = "lot+" in notes
    has_unit_anchor   = bool(re.search(r'unit\+\d+\([^)]{2,}\)', notes) and
                             not re.search(r'unit\+\d+\((NO|NA|1|2|3)\)', notes))
    has_anchor        = has_lot_anchor or has_unit_anchor
    has_strong_subdiv = "subdiv_tok+35" in notes or "exact_legal+10" in notes

    # Surname-only match with no real parcel/lot anchor → cap at LOW
    # Prevents subdivision name words (ROSE, HILL, LAKE) being treated as surnames
    has_parcel_anchor = "direct_parcel" in notes or "strict_legal" in notes
    surname_only      = "surname" in notes and not has_anchor and not has_parcel_anchor

    if surname_only:
        return "LOW" if score >= 30 else "NONE"

    # no_anchor flag means we have no lot/parcel/unit confirmation → cap at LOW
    if "no_anchor" in notes and not has_parcel_anchor:
        return "LOW" if score >= 30 else "NONE"
...
# Document numbers to trace through strict matcher for production verification
_DEBUG_DOC_NUMBERS = {'20260267650'}

def strict_legal_match_row(legal_desc, nal_row_idx, filing_party='', doc_number=''):
    """
    ...
    """
    debug = doc_number in _DEBUG_DOC_NUMBERS
    if debug:
        log.info("[DEBUG %s] strict_legal_match_row called | legal=%r | filing_party=%r",
                 doc_number, legal_desc, filing_party)
    ...
    if debug:
        log.info("[DEBUG %s] parsed: lot=%r phrase=%r core_tokens=%r",
                 doc_number, lot, phrase, core)

    if not phrase or not lot:
        if debug:
            log.info("[DEBUG %s] FALLTHROUGH: no lot or phrase", doc_number)
    ...
    if debug:
        log.info("[DEBUG %s] Step1 S_LEGAL candidates: %d found for tokens %r",
                 doc_number, len(subdiv_candidates), core)
        for r in subdiv_candidates[:5]:
            log.info("[DEBUG %s]   candidate: parcel=%s s_legal=%r addr=%s",
                     doc_number, r.get('PARCEL_ID',''), r.get('S_LEGAL',''), r.get('PHY_ADDR1',''))

    if not subdiv_candidates:
        if debug:
            log.info("[DEBUG %s] FALLTHROUGH: no S_LEGAL subdivision match", doc_number)
        return None  # No subdivision match — fall through

    # Step 2: Parcel suffix lot filter
    suffixes = _lot_to_suffixes(lot)
    if debug:
        log.info("[DEBUG %s] Step2 suffixes tried: %s", doc_number, sorted(suffixes))
    ...
    if debug:
        log.info("[DEBUG %s] Step2 lot candidates: %d (matched suffix=%r)",
                 doc_number, len(lot_candidates), matched_suffix)
        for r in lot_candidates[:3]:
            log.info("[DEBUG %s]   lot_candidate: parcel=%s addr=%s owner=%s",
                     doc_number, r.get('PARCEL_ID',''), r.get('PHY_ADDR1',''), r.get('OWN_NAME',''))

    if not lot_candidates:
        if debug:
            log.info("[DEBUG %s] FALLTHROUGH: no parcel suffix match from %d subdiv candidates",
                     doc_number, len(subdiv_candidates))
    ...
    if debug:
        log.info("[DEBUG %s] STRICT MATCH SUCCESS: parcel=%s addr=%s %s FL %s owner=%s",
                 doc_number, parcel, addr, city, zipcd, owner)
    ...
    for lead in leads:
        ...
        if not direct_result and lead.legal_description:
            strict_result = strict_legal_match_row(
                ...
                doc_number=lead.document_number or ''
            )
