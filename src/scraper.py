...
# Google Drive NAL file - direct download link
NAL_GDRIVE_ID  = "1X1nZkK07FJV3BmUFHUFvpZA1hLEl4UP9"
NAL_GDRIVE_URL = f"https://drive.google.com/uc?export=download&id={NAL_GDRIVE_ID}"
...
def download_nal_file():
    """Download the NAL appraisal file from Google Drive."""
    ...
    try:
        # Google Drive large file needs confirmation token
        session = requests.Session()
        resp = session.get(NAL_GDRIVE_URL, stream=True, timeout=60)
        ...
        if token:
            url = f"{NAL_GDRIVE_URL}&confirm={token}"
            resp = session.get(url, stream=True, timeout=60)
        
        # Also handle the new Google Drive confirmation page
        ...
        with open(NAL_LOCAL_PATH, "wb") as f:
            downloaded = 0
        ...
        return size > 1000000  # Must be at least 1MB
...
def load_nal_index():
    """
    Load the NAL file and build a lookup index by subdivision name.
    Returns dict: subdivision_name -> list of {property_address, mailing_address, owner_name, assessed_value}
    """
    ...
    try:
        with open(NAL_LOCAL_PATH, encoding="utf-8", errors="replace") as f:
            ...
            for row in reader:
                ...
                # Mailing address fields (where owner actually gets mail)
                ...
                # Legal description for matching
                ...
                # Index by subdivision name extracted from legal description
                # Legal desc format: "LOT 146 SHERWOOD PHASE TWO" or "UNIT 4111 CROSSINGS AT CONWAY"
                # We extract the subdivision part (everything after lot/unit number)
        ...
        log.info("NAL index built: %d subdivision keys, %d total records", len(index), count)
        return index

    except Exception as e:
        log.error("NAL index build failed: %s", e)
        return {}

def extract_subdivision(legal_desc):
    """
    Extract the subdivision name from a legal description.
    Examples:
      "LOT 146 SHERWOOD PHASE TWO" -> "SHERWOOD PHASE TWO"
      "UNIT 4111 CROSSINGS AT CONWAY" -> "CROSSINGS AT CONWAY"
      "LOT 181 DIAMOND COVE UNIT 2" -> "DIAMOND COVE"
      "LOT 5 EATON ESTATES UNIT TWO" -> "EATON ESTATES"
    """
    if not legal_desc:
        return ""
...
def match_lead_to_nal(lead, nal_index):
    """
    Match a lead's legal description against the NAL index.
    ...
    # Try progressively broader subdivision extractions
    ...
    if subdiv in nal_index:
        ...
        # Try to find the specific lot
        lot_match = extract_lot_number(legal)
        if lot_match:
            for rec in records:
                if lot_match in rec["s_legal"]:
                    return rec
        # Return first record if only one
        if len(records) == 1:
            return records[0]
        # Return first record as best guess
        if records:
            return records[0]

    # Try partial match — check if any index key is contained in our legal desc
    for key in nal_index:
        if len(key) >= 6 and key in legal:
...
def extract_lot_number(legal_desc):
    """Extract lot number from legal description for precise matching."""
    m = re.search(r'LOT\s+(\w+)', legal_desc.upper())
    if m:
        return f"LOT {m.group(1)}"
    m = re.search(r'UNIT\s+(\w+)', legal_desc.upper())
    if m:
        return f"UNIT {m.group(1)}"
    return ""

def search_and_get_data(session, doc_type, doc_code, doc_label):
    """POST search, return CSV text."""
    payload = {
...
def enrich_leads_with_nal(leads, nal_index):
    """Match each lead against the NAL index to get property/mailing addresses."""
    matched = 0
    for lead in leads:
...
def save_csv(leads, path="data/output.csv"):
    """Save leads to CSV for CRM upload."""
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        for lead in leads:
            row = asdict(lead) if isinstance(lead, Lead) else lead
...
def main():
    ...
    # Step 1: Download NAL file and build index
    nal_index = {}
    if download_nal_file():
        nal_index = load_nal_index()
    else:
        log.warning("NAL file unavailable — addresses will be missing")
    ...
    # Step 3: Enrich new leads with NAL addresses
