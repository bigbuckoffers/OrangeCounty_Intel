...
def _parse_legal_strict(raw):
    ...
    result = {'lot':'','block':'','unit':'','phase':'','subdivision':'','full_phrase':'','raw_norm':text}
    ...
    subdiv = text
    for pat in [r'\bLOT\s+\d+\b',r'\bBLOCK\s+\w+\b',r'\bUNIT\s+\w+\b',r'\bPHASE\s+\w+\b',r'\bSECTION\s+\w+\b']:
...
def _s_legal_matches(s_legal, full_phrase):
    """All significant tokens in full_phrase must appear in S_LEGAL."""
    if not s_legal or not full_phrase:
        return False
    tokens = [t for t in full_phrase.upper().split()
              if t not in _STOP_WORDS_LEGAL and len(t) >= 1]
    if not tokens:
        return False
...
class NALRowIndex:
    ...
    def find_by_s_legal(self, full_phrase):
        """Find all rows where S_LEGAL matches all tokens in full_phrase."""
        tokens = [t for t in full_phrase.upper().split()
                  if t not in _STOP_WORDS_LEGAL and len(t) >= 4]
        if not tokens:
            return []
        # Intersect candidate sets from token index
        ...
        for s in candidate_sets[1:]:
            ...
            if not common:
                break
        # Final verification — all tokens must match
        results = []
        for i in common:
            row = self.rows[i]
            if _s_legal_matches(row.get('S_LEGAL',''), full_phrase):
                results.append(row)
        return results
...
def strict_legal_match_row(legal_desc, nal_row_idx):
    """
    PRIMARY matching method — strict structured legal description lookup.
    Returns result dict with confidence and address fields.
    Only works for subdivision lots (condo units use fallback fuzzy matcher).
    """
    ...
    if not phrase or not lot:
        return None  # Fall through to fuzzy matcher

    subdiv_candidates = nal_row_idx.find_by_s_legal(phrase)
    ...
    if len(lot_candidates) > 1:
        # Ambiguous — don't auto-pick, fall through to fuzzy
        log.debug("Strict match AMBIGUOUS: %d candidates for lot %s in %s", len(lot_candidates), lot, phrase)
        return None
    ...
    log.debug("Strict match HIGH: parcel=%s addr=%s [legal=%s lot=%s suffix=%s]",
              parcel, addr, legal_desc[:50], lot, matched_suffix)

    return {
        "match_confidence": "HIGH",
        "match_score":      100,
        "match_reason":     f"strict:S_LEGAL={s_leg[:40]}+suffix={matched_suffix}|lot={lot}|subdiv={phrase[:40]}",
        "property_address": f"{addr}, {city}, {state} {zipcd}".strip(', '),
        "mailing_address":  "",  # NAL doesn't have mailing — OCPA will fill via parcel
        "owner_name":       owner,
        "assessed_value":   av,
        "parcel_id":        parcel,
        "nal_s_legal":      s_leg,
...
def enrich_leads(leads, nal_idx):
    ...
    for lead in leads:
        ...
        if lead.legal_description:
            strict_result = strict_legal_match_row(lead.legal_description, nal_row_idx)
