...
class Lead:
    ...
    mailing_address:   str  = ""
    owner_name:        str  = ""  # courthouse filing party/contact name
    current_ocpa_owner_name: str = ""  # current owner from NAL/OCPA, kept separate
    assessed_value:    str  = ""
...
def merge_two_leads(primary, secondary):
    ...
    for field in ['property_address', 'prop_street', 'prop_city', 'prop_state',
                  ...
                  'mail_state', 'mail_zip', 'owner_name', 'current_ocpa_owner_name', 'assessed_value',
        ...
        with open(path, encoding="utf-8", errors="replace") as f:
            ...
            for row in reader:
                ...
                record = {
                    ...
                    "parsed":           parsed,
                    "tokens":           tokens,
                    "prop_street":      phy_addr1,
                    "prop_city":        phy_city,
                    "prop_state":       phy_state,
                    "prop_zip":         phy_zip,
                    # Raw-style keys used by strict matcher
                    "PARCEL_ID":        parcel_id,
                    "S_LEGAL":          s_legal,
                    "PHY_ADDR1":        phy_addr1,
                    "PHY_ADDR2":        phy_addr2,
                    ...
                    "TV_NSD":           (row.get("TV_NSD") or "").strip(),
                }
...
def match_lead(lead, nal_idx):
    ...
    return {
        ...
        "current_ocpa_owner_name": best_rec["owner_name"],
        "assessed_value":   best_rec["assessed_value"],
        "parcel_id":        best_rec.get("parcel_id", ""),
        "nal_s_legal":      best_rec.get("s_legal", ""),
        "prop_street":      best_rec.get("prop_street", ""),
        "prop_city":        best_rec.get("prop_city", ""),
        "prop_state":       best_rec.get("prop_state", "FL"),
        "prop_zip":         best_rec.get("prop_zip", ""),
    }
...
def _extract_direct_parcels_from_legal(raw):
    """
    Extract direct Orange County parcel IDs embedded in courthouse legal text.
    Example: "Parcel: 25 23 27 1213 00 250" -> "252327121300250".
    Direct parcel match is the strongest enrichment path.
    """
    if not raw:
        return []
    text = raw.upper()
    ...
    found = []
    patterns = [
        # PARCEL 25 23 27 1213 00 250
        r'\bPARCEL\s+(\d{2})\s+(\d{2})\s+(\d{2})\s+(\d{4})\s+(\d{2})\s+(\d{3,4})\b',
        # 25 23 27 1213 00 250 (only when it appears after the word PARCEL somewhere nearby is preferred above)
    ]
    for pat in patterns:
        for m in re.finditer(pat, text):
            pid = ''.join(m.groups())
            if pid.isdigit() and 14 <= len(pid) <= 16:
                found.append(pid)
...
def _row_to_strict_result(row, match_basis, match_reason, match_score=100, match_confidence="HIGH"):
    """Convert a raw/record NAL row into the standard enrichment result shape."""
    addr   = (row.get('PHY_ADDR1') or row.get('prop_street') or '').strip()
    city   = (row.get('PHY_CITY')  or row.get('prop_city') or '').strip()
    state  = (row.get('PHY_STATE') or row.get('prop_state') or 'FL').strip() or 'FL'
    zipcd  = str(row.get('PHY_ZIPCD') or row.get('prop_zip') or '').strip()[:5]
    parcel = (row.get('PARCEL_ID') or row.get('parcel_id') or '').strip()
    owner  = (row.get('OWN_NAME')  or row.get('owner_name') or '').strip()
    s_leg  = (row.get('S_LEGAL')   or row.get('s_legal') or '').strip()
    jv     = (row.get('JV') or row.get('AV_NSD') or row.get('TV_NSD') or '').strip()
    mailing = (row.get('mailing_address') or '').strip()
    ...
    return {
        ...
        "match_reason":     match_reason[:240],
        "match_basis":      match_basis,
        "property_address": f"{addr}, {city}, {state} {zipcd}".strip(', '),
        "mailing_address":  mailing,
        "owner_name":       owner,
        "current_ocpa_owner_name": owner,
        "assessed_value":   av,
        "parcel_id":        parcel,
        "nal_s_legal":      s_leg,
        "prop_street":      addr,
...
class NALRowIndex:
    """Lightweight index over NAL rows for strict/direct matching."""
    def __init__(self, rows):
        self.rows = rows
        self._token_idx = defaultdict(list)
        self._parcel_idx = defaultdict(list)
        for i, row in enumerate(rows):
            pid = clean_parcel(row.get('PARCEL_ID') or row.get('parcel_id') or '')
            if pid:
                self._parcel_idx[pid].append(i)

            s = (row.get('S_LEGAL') or row.get('s_legal') or '').upper()
            tokens = set(re.findall(r'[A-Z0-9]+', s))
            for tok in tokens:
                if len(tok) >= 3:
                    self._token_idx[tok].append(i)

    def find_by_parcel(self, parcel_id):
        """Exact NAL PARCEL_ID lookup."""
        pid = clean_parcel(parcel_id)
        return [self.rows[i] for i in self._parcel_idx.get(pid, [])]

    def find_by_s_legal(self, full_phrase, core_tokens=None):
        """Find all rows where S_LEGAL contains all required core tokens."""
        tokens = core_tokens if core_tokens else [
            t for t in (full_phrase or '').upper().split()
            if t not in _STOP_WORDS_LEGAL and len(t) >= 3 and not t.isdigit()
        ]
        ...
        for i in common:
            row = self.rows[i]
            if _s_legal_matches(row.get('S_LEGAL') or row.get('s_legal') or '', full_phrase, core_tokens=tokens):
                results.append(row)
        return results
...
def is_condo_lead(lead):
    ...
    Works for both Lead dataclass objects and dicts loaded from existing JSON.
    HOA records are NOT excluded — only condo/unit/apartment/resort records.
    """
    legal   = (get_lead_val(lead, 'legal_description', '') or '').upper()
    grantee = (get_lead_val(lead, 'grantee', '') or '').upper()
    grantor = (get_lead_val(lead, 'grantor', '') or '').upper()

    # Legal description contains explicit condo/apartment/timeshare keywords.
    # Do NOT exclude "UNIT" alone here — many SFR subdivisions use "UNIT ONE".
    ...
    # Unit-only legal descriptions with no LOT are usually condos/apartments.
    # Example: "Unit 203 Central Park". Do not exclude "Lot 39 ... Unit One".
    if re.search(r'UNIT\s+\d+[A-Z]?', legal) and not re.search(r'LOT\s+\d+', legal):
        return True, "Unit-based legal description with no lot number"
...
def strict_legal_match_row(legal_desc, nal_row_idx, filing_party=''):
    ...
    Priority:
      1. Direct parcel extraction from legal description -> exact PARCEL_ID match.
      2. Core S_LEGAL tokens -> candidate pool.
      3. Parcel suffix from lot number -> property-level anchor.
      4. Filing party/current owner confirmation -> tiebreaker.

    Returns result dict or None. Returning None means the lead should fall back
    to fuzzy/manual matching. Never returns an address from subdivision alone.
    """
    # ── 1) Direct parcel match: strongest path ───────────────────────────
    direct_parcels = _extract_direct_parcels_from_legal(legal_desc)
    for pid in direct_parcels:
        hits = nal_row_idx.find_by_parcel(pid)
        if len(hits) == 1:
            return _row_to_strict_result(
                hits[0],
                match_basis="direct_parcel",
                match_reason=f"direct:parcel={pid}|exact NAL PARCEL_ID match",
                match_score=100,
            )
        elif len(hits) > 1 and filing_party:
            party_tokens = [t for t in filing_party.upper().split() if len(t) >= 4]
            confirmed = [
                r for r in hits
                if any(tok in (r.get('OWN_NAME') or r.get('owner_name') or '').upper() for tok in party_tokens)
            ]
            if len(confirmed) == 1:
                return _row_to_strict_result(
                    confirmed[0],
                    match_basis="direct_parcel_owner_confirmed",
                    match_reason=f"direct:parcel={pid}|owner confirmed",
                    match_score=100,
                )

    # ── 2) Lot/subdivision strict match ──────────────────────────────────
    ...
    for suffix in suffixes:
        hits = [
            ...
            if clean_parcel(r.get('PARCEL_ID') or r.get('parcel_id') or '').endswith(suffix)
        ]
        if hits:
            lot_candidates = hits
    ...
    # Owner confirmation is only a tiebreaker, never the primary match key.
    ...
    if len(lot_candidates) != 1:
        log.debug("Strict match unresolved: %d candidates for lot=%s core=%s legal=%s",
                  len(lot_candidates), lot, core, legal_desc[:80])
        return None
    ...
    return _row_to_strict_result(
        row,
        match_basis="strict_legal_lot_suffix",
        match_reason=f"strict:S_LEGAL={s_leg[:40]}|suffix={matched_suffix}|lot={lot}|core={','.join(core[:5])}",
        match_score=100,
    )
...
def build_nal_row_index(nal_idx):
    ...
    rows = list(nal_idx.records.values())
...
def enrich_leads(leads, nal_idx):
    ...
    for lead in leads:
        ...
        # ── PRIMARY: Strict structured legal match ────────────────────────
        ...
        if strict_result:
            # Strict match succeeded — use it
            lead.match_confidence  = strict_result["match_confidence"]
            lead.match_score       = strict_result["match_score"]
            lead.match_reason      = strict_result["match_reason"]
            lead.property_address  = strict_result["property_address"]
            lead.mailing_address   = strict_result.get("mailing_address", lead.mailing_address)
            lead.current_ocpa_owner_name = strict_result.get("current_ocpa_owner_name", "")
            lead.assessed_value    = strict_result["assessed_value"]
            lead.needs_enrichment  = False
            lead.parcel_id         = strict_result.get("parcel_id", "")
            lead.nal_s_legal       = strict_result.get("nal_s_legal", "")
            lead.prop_street       = strict_result.get("prop_street", "")
            lead.prop_city         = strict_result.get("prop_city", "")
            lead.prop_state        = strict_result.get("prop_state", "FL")
            lead.prop_zip          = strict_result.get("prop_zip", "")
            lead.county_search_url = build_county_search_url(lead)
            # Never overwrite filing party with NAL owner
            if not lead.owner_name and strict_result.get("owner_name"):
                lead.owner_name = strict_result["owner_name"]
            strict_hits += 1
            ...
            lead.current_ocpa_owner_name = result.get("current_ocpa_owner_name", result.get("owner_name", ""))
            lead.assessed_value    = result["assessed_value"]
            lead.parcel_id         = result.get("parcel_id", "")
            lead.nal_s_legal       = result.get("nal_s_legal", "")
            lead.prop_street       = result.get("prop_street", "")
            lead.prop_city         = result.get("prop_city", "")
            lead.prop_state        = result.get("prop_state", "FL")
            lead.prop_zip          = result.get("prop_zip", "")
            lead.county_search_url = build_county_search_url(lead)
    ...
    log.info(
        "Match results -> HIGH:%d  MEDIUM:%d  LOW:%d  NONE:%d | strict=%d fuzzy=%d",
        counts["HIGH"], counts["MEDIUM"], counts["LOW"], counts["NONE"],
        strict_hits, fuzzy_hits
    )
    return leads
...
def save_csv(leads, path="data/output.csv"):
    ...
    fields = [
        ...
        "property_address", "mailing_address", "owner_name", "current_ocpa_owner_name", "assessed_value",
        "match_confidence", "match_score", "match_reason", "county_search_url",
        "distress_flags", "needs_enrichment", "scraped_at", "nal_s_legal",
...
def main():
    ...
    # ── Filter out condo/unit properties before enrichment/export ─────────
    # Do this before NAL matching and again for existing JSON so old condo rows
    # do not remain in the dashboard/export.
    all_excluded_condos = []
    if new_leads:
        new_leads, excluded_condos = filter_condo_leads(new_leads)
        all_excluded_condos.extend(excluded_condos)
    ...
    if existing:
        existing, old_excluded = filter_condo_leads(existing)
        all_excluded_condos.extend(old_excluded)

    save_excluded_condos(all_excluded_condos)
