...
def search_and_get_csv(session, doc_type, doc_code, doc_label):
    ...
    try:
        ...
        log.info("CSV preview: %s", csv_resp.text[:500])
...
def parse_csv_text(csv_text, doc_type, base_score):
    ...
    # Find the header row — look for the line with "Document #"
    header_idx = None
    for i, line in enumerate(lines):
        if '"Document #"' in line or 'Document #' in line:
            header_idx = i
            log.info("Found CSV header at line %d: %s", i, line[:100])
            break

    if header_idx is None:
        log.error("Could not find header row. Full CSV:\n%s", csv_text[:1000])
        return []
    ...
    for row in reader:
        # Strip quotes and whitespace from all values
        row = {k: v.strip().strip('"') if v else "" for k, v in row.items()}
        doc_num = row.get("Document #") or row.get("Document") or ""
        ...
        leads.append(Lead(
            ...
            property_address=row.get("Legal", ""),
            document_type=dtype,
            seller_score=score,
            distress_flags=flags,
        ))

    log.info("Parsed %d leads from CSV", len(leads))
    return leads
