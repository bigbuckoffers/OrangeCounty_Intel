...
TEST_CASES = [
    ...
    {
        "input":            "Unit: 203 CENTRAL PARK A METROWEST CONDOMINIUM",
        "filing_party":     "RODRIGUES ZACKARY",
        "expected_address": "",
        "expected_city":    "",
        "expected_zip":     "",
        "expected_parcel":  "",
...
def strict_legal_match(legal_desc, nal_rows, verbose=True):
    ...
    # If no lot but have unit, try unit-based suffixes too
    unit = parsed.get('unit', '')
    if not lot and unit:
        try:
            n = int(unit)
            unit_suffixes = [
                str(n * 10).zfill(3),  # unit × 10
                str(n).zfill(3),        # zero-padded
                str(n).zfill(4),
                str(n),                 # raw
            ]
                ...
                for r in nal_rows:
                    ...
                    if all(t in s for t in tokens):
                        ...
                        if shown >= 20:
                            ...
                            break
                if shown == 0:
                    print(f"    No rows found containing all of {tokens}")
                    # Try with fewer tokens
                    for t in tokens:
                        sample = [r for r in nal_rows if t in (r.get('S_LEGAL') or '').upper()][:5]
                        print(f"\n    Rows containing just '{t}' ({len(sample)} shown of many):")
                        for r in sample:
                            print(f"      PARCEL: {r.get('PARCEL_ID','')} | S_LEGAL: {(r.get('S_LEGAL') or '')[:80]} | PHY_ADDR1: {r.get('PHY_ADDR1','')}")
                discovery += 1
