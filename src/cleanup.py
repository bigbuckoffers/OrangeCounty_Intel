...
def _build_row_index(rows):
    ...
    Normalizes hyphens to spaces so 'N-25B' matches '25B' token.
    """
    from collections import defaultdict
    token_idx = defaultdict(list)
    for i, row in enumerate(rows):
        s = (row.get("S_LEGAL") or "").upper()
        # Normalize: replace hyphens with spaces so "N-25B" -> "N 25B" -> token "25B"
        s = s.replace("-", " ")
        s = " ".join(s.split())  # normalize whitespace
        for tok in set(s.split()):
            if len(tok) >= 2:  # match our core token min length
...
def _s_legal_matches(s_legal, core_tokens):
    ...
    Normalizes hyphens so 'N-25B' matches token '25B'.
    """
    s = s_legal.upper().replace("-", " ")
...
def _parse_legal_for_revalidate(raw):
    ...
    _STRIP = {"REPLAT","PHASE","UNIT","SECTION","PLAT","AMENDED","REVISED",
              "ADDITION","EXTENSION","PARCEL","NO","THE","OF","A",
              "AN","AND","OR","IN","AT","TO","FOR","LOT","LOTS","BLOCK"}
    # NOTE: TRACT is intentionally NOT stripped — it is meaningful in subdivision names
    # e.g. "Waterford Lakes Tract N 25B" — TRACT distinguishes the tract subdivision
    core = [t for t in subdiv.split()
            if t not in _STRIP and len(t) >= 2 and not t.isdigit()]
    return lot, core


def _lot_to_suffixes(lot):
    """
    Returns ORDERED list of parcel suffix candidates.
    lot*10 variants first (standard OC format: Lot 5 -> suffix 050).
    Raw lot variants second (fallback only).
    Order matters — first match wins.
    """
    base = lot * 10
    return [
        str(base).zfill(3),   # 050  ← standard OC format, try first
        str(base).zfill(4),   # 0050
        str(base).zfill(5),   # 00050
        str(base).zfill(6),   # 000050
        str(lot).zfill(3),    # 005  ← fallback only
        str(lot).zfill(4),    # 0005
    ]
...
def _extract_direct_parcel(legal):
    ...
    m = re.search(
        r"\bPARCEL\s*:?\s*(\d{2})\s+(\d{2})\s+(\d{2})\s+(\d{4})\s+(\d{2})\s+(\d{3,4})\b",
        upper
    )
    if m:
...
# Documents to trace through strict matcher
_DEBUG_DOCS = {"20260267650", "20260237090", "20260268884", "20260268923", "20260264722"}
...
def _strict_match(legal, filing_party, rows, token_idx, doc_number="",
    ...
    """
    debug = doc_number in _DEBUG_DOCS
        ...
        for row in rows:
            ...
            if rp == clean:
                ...
                return (row["PARCEL_ID"], row["PHY_ADDR1"], row["PHY_CITY"],
                        ...
                        row["OWN_NAME"], row.get("AV_NSD",""), "direct_parcel", row)
        if debug:
    ...
    # Fallback: if no candidates with alphanumeric tokens, try core tokens without them
    if not subdiv_candidates:
        alpha_core = [t for t in core if t.isalpha()]
        if len(alpha_core) < len(core) and len(alpha_core) >= 2:
            alpha_sets = [set(token_idx.get(t, [])) for t in alpha_core]
            if alpha_sets:
                alpha_common = alpha_sets[0]
                for s in alpha_sets[1:]:
                    alpha_common &= s
                subdiv_candidates = [rows[i] for i in alpha_common
                                     if _s_legal_matches(rows[i].get("S_LEGAL",""), alpha_core)]
                if subdiv_candidates and debug:
                    log.info("[DEBUG %s] Fallback alpha-only core %s found %d candidates",
                             doc_number, alpha_core, len(subdiv_candidates))
    ...
    if len(lot_candidates) > 1 and filing_party:
        ...
        if confirmed:
            ...
            # Multiple candidates, owner confirmation failed — do NOT guess
            # Returning None prevents overwriting a good existing match
            if debug:
                log.info("[DEBUG %s] FALLTHROUGH: %d candidates but owner confirmation "
                         "failed — not guessing", doc_number, len(lot_candidates))
            # Extra safety: if fallback produced too many candidates (alpha-only),
            # definitely do not proceed
            if len(lot_candidates) > 5:
                if debug:
                    log.info("[DEBUG %s] FALLTHROUGH: too many ambiguous candidates (%d)",
                             doc_number, len(lot_candidates))
                return None
            # For small candidate sets (2-5), still return None — owner must confirm
            return None
    ...
    return (chosen["PARCEL_ID"], chosen["PHY_ADDR1"], chosen["PHY_CITY"],
            ...
            chosen["OWN_NAME"], chosen.get("AV_NSD",""), "strict_legal", chosen)
        ...
        with open(NAL_LOCAL_PATH, encoding="utf-8", errors="replace") as f:
            ...
            for row in reader:
                ...
                rows.append({
                    ...
                    "OWN_ZIPCD": str(row.get("OWN_ZIPCD") or "").strip()[:5],
                })
    except Exception as e:
...
_RELIABLE_MAILING_SOURCES = {'tax delinquent', 'code violation', 'foreclosure'}

def _is_parcel_confirmed(lead):
    """
    Returns True only if the parcel/address on this lead is confirmed reliable.
    Wrong mailing is worse than blank mailing — only fill when we trust the parcel.
    """
    confidence = (lead.get('match_confidence') or '').upper()
    reason     = (lead.get('match_reason') or '').lower()
    pid        = clean_parcel(lead.get('parcel_id') or '')
    doc_type   = (lead.get('document_type') or '').lower()

    # Must have a valid parcel and a confirmed property address
    if not is_valid_parcel(pid):
        return False
    if not lead.get('property_address') and not lead.get('prop_street'):
        return False

    # Explicit confirmation signals
    if 'direct_parcel'        in reason: return True
    if 'strict_legal'         in reason: return True
    if 'revalidated by cleanup' in reason: return True

    # Reliable parcel-based sources at HIGH/MEDIUM confidence
    if confidence in ('HIGH', 'MEDIUM'):
        if any(s in doc_type for s in _RELIABLE_MAILING_SOURCES):
            return True

    # HIGH/MEDIUM with no fuzzy/ambiguous signals
    if confidence in ('HIGH', 'MEDIUM'):
        unsafe = ('ambiguous' in reason or 'no_anchor' in reason or
                  ('surname' in reason and 'strict' not in reason
                   and 'direct' not in reason))
        if not unsafe:
            return True
...
def fill_missing_mailing(leads):
    """
    Fill missing mailing address ONLY for leads with a confirmed reliable parcel.
    Wrong mailing is worse than blank mailing.
    Safe criteria: direct_parcel / strict_legal / revalidated / reliable source HIGH/MEDIUM.
    """
    _RELIABLE_SOURCES = {'tax delinquent', 'code violation', 'foreclosure'}

    # Separate confirmed vs unconfirmed leads missing mailing
    needs_mail    = []
    skip_unconf   = 0
    for lead in leads:
        if not isinstance(lead, dict):
            continue
        if lead.get('mail_street') or lead.get('mailing_address'):
            continue  # already has mailing
        if _is_parcel_confirmed(lead):
            needs_mail.append(lead)
        else:
            # Mark as skipped — unconfirmed parcel
            if not lead.get('mailing_source'):
                lead['mailing_source']    = 'skipped_unconfirmed_parcel'
                lead['needs_skiptrace']   = False
                lead['skiptrace_status']  = 'needs_property_review_first'
            skip_unconf += 1

    log.info("Mailing fill: %d confirmed parcel leads to fill | %d skipped unconfirmed",
             len(needs_mail), skip_unconf)
    ...
    filled = 0
    for lead in needs_mail:
        pid = clean_parcel(lead.get('parcel_id', ''))
        row = parcel_lookup.get(pid)
        if not row:
            continue
        own1 = (row.get('OWN_ADDR1') or '').strip()
        own2 = (row.get('OWN_ADDR2') or '').strip()
        oc   = (row.get('OWN_CITY')  or '').strip()
        os_  = (row.get('OWN_STATE') or 'FL').strip() or 'FL'
        oz   = str(row.get('OWN_ZIPCD') or '').strip()[:5]
        if not own1:
            continue
        mail_street = (own1 + (' ' + own2 if own2 else '')).strip()
        lead['mail_street']    = mail_street
        lead['mail_city']      = oc
        lead['mail_state']     = os_
        lead['mail_zip']       = oz
        parts = [mail_street]
        if oc:         parts.append(oc)
        if os_ and oz: parts.append(f"{os_} {oz}")
        lead['mailing_address'] = ', '.join(parts)
        lead['mailing_source']  = 'NAL_by_confirmed_parcel'
        filled += 1

    log.info("Filled mailing address for %d leads | %d had no NAL row", 
             filled, len(needs_mail) - filled)
    return leads

def fill_missing_city_zip(leads):
    """
    For records with a valid parcel ID but missing prop_city or prop_zip,
    look them up in NAL by parcel ID and fill in the correct city/zip.
    Also validates suspicious cities (street names used as city).
    """
    _VALID_OC_CITIES = {
    ...
    if not needs_fix:
        log.info("No leads need city/zip fill")
        return leads
    ...
    for lead in needs_fix:
        ...
        if not city or city.upper() not in _VALID_OC_CITIES:
            city = 'Orlando'

        old_city = lead.get('prop_city')
        lead['prop_city']  = city
        lead['prop_state'] = row.get('PHY_STATE') or 'FL'
        if zipcd:
            lead['prop_zip'] = zipcd

        # Rebuild property_address with correct city
        street = lead.get('prop_street') or ''
        state  = lead.get('prop_state') or 'FL'
        if street and city:
            lead['property_address'] = f"{street}, {city}, {state} {zipcd}".strip()

        if old_city != city:
            fixed += 1

    log.info("Filled city/zip for %d leads", fixed)
    return leads
...
def revalidate_with_nal(leads):
    ...
    for lead in leads:
        ...
        parcel, addr1, city, state, zipcd, own_name, av, source, *_row = result
        chosen_row = _row[0] if _row else None
        ...
        # SAFETY: never overwrite an existing confirmed strict/direct match
        # with a new result that has a different parcel, unless the new result
        # had owner confirmation. Prevents regression like NNADI.
        if should_update and old_is_strict and new_parcel != old_parcel:
            # New result found a different parcel than the confirmed one
            # Only allow if new result came via direct_parcel (unambiguous)
            if source != "direct_parcel":
                if debug:
                    log.info("[DEBUG %s] SAFETY BLOCK: not overwriting confirmed %s with "
                             "unconfirmed new parcel %s (source=%s)",
                             doc_num, old_parcel, new_parcel, source)
                skipped += 1
                continue
...
def is_address_stackable(lead):
    """
    A lead is only eligible for address-based stacking if it has a valid parcel ID.
    No parcel ID = no address stacking, regardless of confidence or document type.
    This prevents LOW confidence fuzzy matches from merging unrelated properties.
    """
    return is_valid_parcel(clean_parcel(lead.get('parcel_id', '') or lead.get('parcel_id', '')))
...
def unstack_bad_merges(leads):
    """
    Find records that were stacked but should not have been.
    A stacked record with no valid parcel ID and LOW/NONE confidence
    was almost certainly merged incorrectly by address.
    Split those back into their component documents using stacked_docs.
    """
    result = []
    unstacked = 0
    ...
    for lead in leads:
        ...
        # Only examine stacked records
        if not lead.get('stacked'):
            result.append(lead)
            continue
        ...
        # A stacked record is safe if it has a valid parcel ID
        if is_valid_parcel(pid):
            result.append(lead)
            continue

        # Stacked with no parcel + LOW/NONE/fuzzy = bad merge
        # Just un-mark it as stacked — the re-stack pass will handle it correctly
        if confidence in ('LOW', 'NONE', 'NEEDS_REVIEW') or            ('strict' not in reason and 'direct' not in reason and 'revalidated' not in reason):
            lead['stacked']          = False
            lead['stacked_docs']     = []
            lead['stacked_types']    = []
            lead['motivation_count'] = 1
            # Reset document_type to first type only
            types = [t.strip() for t in (lead.get('document_type') or '').split(' + ')]
            if types:
                lead['document_type'] = types[0]
            unstacked += 1

        result.append(lead)

    log.info("Unstacked %d bad merges", unstacked)
    return result


def final_address_validation_guard(leads):
    """
    Hard safety rule: no address stays on a lead unless it was confirmed by:
    1. direct_parcel match
    2. strict_legal match  
    3. revalidated by cleanup
    4. reliable source (tax/code/foreclosure) WITH valid parcel_id
    Any lead with NONE/LOW/NEEDS_REVIEW confidence that doesn't meet these
    criteria gets its address cleared.
    """
    _RELIABLE_SOURCES = {'tax delinquent', 'code violation', 'foreclosure'}
    cleared = 0
    ...
    for lead in leads:
        ...
        # Already clean — skip
        if confidence in ('HIGH', 'MEDIUM', 'SKIPPED_CONDO'):
            continue

        # Check validation proofs
        has_direct    = 'direct_parcel' in reason
        has_strict    = 'strict_legal'  in reason
        has_revalidated = 'revalidated by cleanup' in reason
        ...
        if doc_num in _DEBUG_DOCS:
            log.info("[DEBUG %s] address_guard: confidence=%s has_direct=%s has_strict=%s "
                     "has_revalidated=%s has_reliable=%s confirmed=%s addr=%r",
                     doc_num, confidence, has_direct, has_strict,
                     has_revalidated, has_reliable, confirmed,
                     lead.get('property_address'))

        if not confirmed and (lead.get('property_address') or lead.get('prop_street')):
            if doc_num in _DEBUG_DOCS:
                log.info("[DEBUG %s] address_guard: CLEARING stale address %r parcel=%s",
                         doc_num, lead.get('property_address'), lead.get('parcel_id'))
            lead['property_address'] = ''
            lead['prop_street']      = ''
            lead['prop_city']        = ''
            lead['prop_state']       = ''
            ...
            lead['needs_enrichment'] = True
            lead['match_confidence'] = 'NEEDS_REVIEW'
            lead['match_reason']     = (lead.get('match_reason') or '') +                 ' | cleared stale unvalidated address before final save'
            cleared += 1

    log.info("Final address guard: cleared %d unvalidated addresses", cleared)

    # Post-guard: strip stacked status from NEEDS_REVIEW leads with no confirmed address
    # These should not appear as high-priority stacked leads
    unstacked_nr = 0
    for lead in leads:
        if not isinstance(lead, dict):
            continue
        # Blank address = no confirmed property (check both address and street)
        no_addr = not lead.get('property_address') and not lead.get('prop_street')
        if (lead.get('match_confidence') == 'NEEDS_REVIEW' and no_addr):
            if lead.get('stacked'):
                lead['stacked']          = False
                lead['stacked_docs']     = []
                lead['stacked_types']    = []
                lead['motivation_count'] = 1
                # Reset to first doc type only
                types = [t.strip() for t in
                         (lead.get('document_type') or '').split(' + ')]
                if types:
                    lead['document_type'] = types[0]
                # Cap score — unverified record should not be high priority
                lead['seller_score'] = min(
                    float(lead.get('seller_score') or 0), 35.0)
                unstacked_nr += 1

    log.info("Post-guard: unstacked %d NEEDS_REVIEW blank-address leads", unstacked_nr)
    return leads


def correct_swapped_parcels(leads, rows):
    """
    Fix OC parcel IDs where Section and Range are swapped.
    OC format: SS-TT-RR-SSSS-BB-LLL
    Section 01-36, Township 19-24, Range 27-33.
    If first 2 digits look like Range (27-33) and positions 4-5 look like Section,
    swap them, verify against NAL, and update the lead.
    """
    if not rows:
        return leads

    # Build parcel lookup from NAL
    parcel_lookup = {}
    for row in rows:
        pid = re.sub(r"[-\s]", "", row.get("PARCEL_ID", ""))
        if pid and pid not in parcel_lookup:
    ...
    for lead in leads:
        ...
        # Already valid
        if 1 <= sec <= 36 and 19 <= twp <= 24 and 27 <= rng <= 33:
            continue

        # Try swapping sec and rng
        swapped = pid[4:6] + pid[2:4] + pid[0:2] + pid[6:]
        if swapped not in parcel_lookup:
            continue

        try:
            s2 = int(swapped[0:2])
            r2 = int(swapped[4:6])
            if not (1 <= s2 <= 36 and 27 <= r2 <= 33):
                continue
        except (ValueError, IndexError):
            continue

        # Swapped parcel exists in NAL — apply correction
        row = parcel_lookup[swapped]
        addr1 = (row.get('PHY_ADDR1') or '').strip()
        city  = (row.get('PHY_CITY')  or '').strip()
        state = (row.get('PHY_STATE') or 'FL').strip() or 'FL'
        zipcd = str(row.get('PHY_ZIPCD') or '').strip()[:5]
        own   = (row.get('OWN_NAME')  or '').strip()
        ...
        if city.upper() not in _VALID_OC_CITIES:
            city = 'Orlando'

        log.info("Corrected swapped parcel: %s -> %s | addr=%s %s %s",
                 pid, swapped, addr1, city, zipcd)

        lead['parcel_id'] = swapped
        if addr1:
            lead['prop_street']       = addr1
            lead['prop_city']         = city
            lead['prop_state']        = state
            lead['prop_zip']          = zipcd
            lead['property_address']  = f"{addr1}, {city}, {state} {zipcd}".strip()
        if own:
            lead['current_ocpa_owner_name'] = own
        lead['match_reason'] = (lead.get('match_reason') or '') +                                 ' | corrected_swapped_parcel_sec_rng'

        # Fix document_number if it was built from old parcel (CV-XXXXXXXXXX)
        doc = lead.get('document_number') or ''
        if doc.startswith('CV-') and pid in doc:
            lead['document_number'] = doc.replace(pid, swapped)

        # Fix county_search_url
        url = lead.get('county_search_url') or ''
        if pid in url:
            lead['county_search_url'] = url.replace(pid, swapped)

        corrected += 1

    log.info("Corrected %d swapped Section/Range parcel IDs", corrected)
    return leads
...
def main():
    ...
    # ── STEP 3b: Fill missing city/zip from NAL by parcel ID ─────────────
    log.info("Step 3b: Filling missing city/zip from NAL by parcel ID...")
    active = fill_missing_city_zip(active)
    gc.collect()

    # ── STEP 3d: Fill missing mailing addresses from NAL ─────────────────
    log.info("Step 3d: Filling missing mailing addresses from NAL...")
    active = fill_missing_mailing(active)
    gc.collect()

    # ── STEP 3e: Correct swapped Section/Range parcel IDs ────────────────
    log.info("Step 3e: Correcting swapped Section/Range parcel IDs...")
    # Reuse the already-loaded NAL rows from fill_missing_city_zip
    _nal_rows_for_parcel_fix = _load_nal_all_rows()
    active = correct_swapped_parcels(active, _nal_rows_for_parcel_fix)
    del _nal_rows_for_parcel_fix
    gc.collect()
    ...
    # ── STEP 5: Re-stack/dedup with safe rules ────────────────────────────
    log.info("Step 5: Stacking/deduplicating %d active leads...", len(active))
    ...
    # ── STEP 8: Final condo exclusion (catches post-stack condos) ────────
    log.info("Step 8: Final condo exclusion pass...")
    final_excluded = []
    final_active   = []
    for lead in active:
        if not isinstance(lead, dict):
            final_active.append(lead)
            continue
        is_condo, reason = is_condo_lead(lead)
        if is_condo:
            lead['exclude_from_dashboard'] = True
            ...
            lead['excluded_reason']        = reason
            lead['match_confidence']       = 'SKIPPED_CONDO'
            final_excluded.append(lead)
        else:
            final_active.append(lead)
    excluded += final_excluded
    active = final_active
    log.info("Final condo pass: %d additional condos excluded", len(final_excluded))

    # ── STEP 9: Final address validation guard ────────────────────────────
    log.info("Step 9: Final address validation guard...")
    active = final_address_validation_guard(active)
