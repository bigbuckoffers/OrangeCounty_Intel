...
print("CLEANUP_VERSION = 2026-05-19-v5-mailing-parcel-cleanup", flush=True)
...
def final_address_validation_guard(leads):
    ...
    for lead in leads:
        if not isinstance(lead, dict):
            continue
        if (lead.get('match_confidence') == 'NEEDS_REVIEW' and
                not lead.get('property_address') and
                not lead.get('prop_street')):
            if lead.get('stacked'):
                lead['stacked']          = False
                lead['stacked_docs']     = []
                lead['stacked_types']    = []
