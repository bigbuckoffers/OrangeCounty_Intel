...
def save_no_match_diagnostics(leads, path="data/no_match_diagnostics.csv"):
    ...
    for lead in leads:
        ...
        if conf in ("LOW", "NONE"):
            ...
            direct = _extract_direct_parcel(legal)
            diag_leads.append({
    ...
    if len(lot_candidates) > 1 and filing_party:
        ...
        if len(confirmed) == 1:
            ...
            log.debug("Owner confirmation resolved %d candidates → 1", len(lot_candidates))
        elif len(confirmed) > 1:
            lot_candidates = confirmed  # Still narrowed, continue

    if len(lot_candidates) > 1:
        # Still ambiguous — fall through to fuzzy
