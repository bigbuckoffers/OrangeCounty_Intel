...
def revalidate_with_nal(leads):
    ...
    for lead in leads:
        ...
        if should_update:
            ...
            # For Lis Pendens: grantee is defendant (person being foreclosed) = motivated seller
            # For Liens/Judgments: grantor is typically the property owner
            doc_type     = (lead.get("document_type") or "").lower()
            grantee      = (lead.get("grantee") or "").strip()
            grantor      = (lead.get("grantor") or "").strip()
            if any(x in doc_type for x in ("lis pendens", "probate", "death")):
                courthouse_owner = grantee or grantor
            else:
                courthouse_owner = grantor or grantee
            ...
            if debug:
                ...
                log.info("[DEBUG %s] owner_name reset: old=%r -> new=%r (courthouse filing party)",
                         doc_num, lead.get("owner_name"), courthouse_owner)
            ...
            # Reset owner_name to courthouse filing party — never let a
            # stacked/merged entity name override the actual property owner
            if courthouse_owner:
