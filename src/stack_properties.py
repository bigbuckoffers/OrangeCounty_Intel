...
"""
import json, logging, os, re, csv, sys, subprocess
from datetime import datetime

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger(__name__)

print("STACK_PROPERTIES_VERSION = 2026-05-15-v2-streaming", flush=True)

OUTPUT_PATH = "data/output.json"
OUTPUT_CSV  = "data/output.csv"
TEMP_PATH   = "data/output_stacked_tmp.json"
...
def merge_into(primary, secondary):
    """Merge secondary record's signals into primary in-place."""
...
def stack_by_property_streaming(ijson):
    """
    Streaming two-pass approach:
    Pass 1: Stream output.json, build stacked output list in memory.
            But critically — we stream the INPUT so json.load() never
            allocates the full source array. The output list is the only
            large structure in memory at any time.
    """
    total = 0
    merged_count = 0
    by_parcel = {}   # clean_parcel -> index in output
    by_addr   = {}   # clean_addr   -> index in output
    output    = []   # final merged records
    meta      = {}   # generated_at, date_range

    log.info("Streaming input from %s ...", OUTPUT_PATH)

    with open(OUTPUT_PATH, 'rb') as f:
        # Extract top-level metadata fields first
        parser = ijson.parse(f)
        for prefix, event, value in parser:
            if prefix == 'generated_at' and event in ('string', 'null'):
                meta['generated_at'] = value
            elif prefix == 'date_range' and event in ('string', 'null'):
                meta['date_range'] = value
            # Stop early once we've hit the leads array start
            elif prefix == 'leads' and event == 'start_array':
                break

    with open(OUTPUT_PATH, 'rb') as f:
        for lead in ijson.items(f, 'leads.item'):
            ...
            total += 1
            if total % 5000 == 0:
                log.info("  Stacking progress: %d leads processed (%d merged so far)", total, merged_count)
...
def main():
    ...
    leads, meta = stack_by_property_streaming(ijson)
    ...
    generated_at = meta.get('generated_at') or (datetime.utcnow().isoformat() + 'Z')
