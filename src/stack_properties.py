...
"""
import json, logging, os, re, csv, sys
from datetime import datetime

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
log = logging.getLogger(__name__)

print("STACK_PROPERTIES_VERSION = 2026-05-15-debug-1", flush=True)
...
def main():
    ...
    # Free original data from memory before writing — avoids holding 3x leads in RAM
    generated_at = datetime.utcnow().isoformat() + 'Z'
    date_range   = data.get('date_range', '')
    del data  # free the original loaded JSON from memory

    log.info("Writing output.json (%d leads)...", len(leads))
    with open(OUTPUT_PATH, 'w', encoding='utf-8') as f:
        # Stream write to avoid building full JSON string in memory
